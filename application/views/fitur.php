        ...
        <div class="box">
          ...
          <div class="box-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
              <?php $no=1; foreach ($fitur as $f): ?>
                <tr>
                  <td><?php echo $no;?></td>
                  <td><?php echo $f->nama;?></td>
                  <td><?php $c=$f->tipe; if ($c == 0) {
                    echo "Hotel";
                  } else if($c == 0.1) {
                    echo "Hotel, Room";
                  }else {
                  ...
                  <td><button class="btn btn-warning fa fa-edit" onclick="editdata(<?php echo $f->id_fitur;?>)"></button>
							        <button class="btn btn-danger fa fa-close" onclick="deletedata(<?php echo $f->id_fitur;?>)"> </button>
                  </td>
                </tr>
              <?php $no++; endforeach; ?>
              </tbody>
...
<script>
  var save_method1; //for save method string
    ...
    function adddata()
    {
      ...
      $("#modalbody").load("modalfr/",function(data){
		      $("#modalbody").html(data);
    ...
    function editdata(id)
    {
      ...
      $('#form')[0].reset(); // reset form on modals
      $('#modal_form').modal('show'); // show bootstrap modal
      $("#modalbody").load("fredit/"+id,function(data){
		      $("#modalbody").html(data);
          //iCheck for checkbox and radio inputs
          $('input[type="checkbox"].minimal, input[type="radio"].minimal').iCheck({
            checkboxClass: 'icheckbox_minimal-blue',
    ...
    function deletedata(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('frdelete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
