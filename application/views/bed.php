        ...
        <div class="box">
          ...
          <div class="box-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
              <?php $no=1; foreach ($bed as $key): ?>
                <tr>
                  <td><?php echo $no;?></td>
                  <td><?php echo $key->bed;?></td>
                  <td><button class="btn btn-warning fa fa-edit" onclick="editdata(<?php echo $key->id_bed;?>)"></button>
							        <button class="btn btn-danger fa fa-close" onclick="deletedata(<?php echo $key->id_bed;?>)"> </button>
                  </td>
                </tr>
              <?php $no++; endforeach; ?>
              </tbody>
...
<script>
  var save_method1; //for save method string
    ...
    function adddata()
    {
      ...
      $("#modalbody").load("modalbed/",function(data){
		      $("#modalbody").html(data);
    ...
    function editdata(id)
    {
      ...
      $('#form')[0].reset(); // reset form on modals
      $('#modal_form').modal('show'); // show bootstrap modal
      $("#modalbody").load("bededit/"+id,function(data){
		      $("#modalbody").html(data);
    ...
    function deletedata(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('beddelete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
